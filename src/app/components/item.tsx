...
const Item = ({props, onUpdate, onDelete, date}) => {
  ...
  return (
    <>
    <li key = {props.id} className="flex flex-row items-center justify-between p-1 min-w-60">
    <p className="text-xs max-w-28">{props.name}</p>
    <div className="flex  items-center ">
      <p className=" text-xs mr-1 min-w-16 text-end">@ {props.price}円</p>
      <button className="border mr-1 p-1 text-xs" onClick={() => handleUse(props.id)}>使</button>
      <button className="border mr-1 p-1 text-xs" onClick={() => handleDelete(props.id)}>消</button>
    </div>
  </li>
    </>
  )
