    ...
    <main>
      ...
      <div className="mb-10">
        ...
        <input
          ...
          />
          <div className="flex flex-row items-center gap-1">
            <input type="radio" id="taxExclude" name="tax" value="税抜" checked={taxNotation.tax === "税抜" } onChange = { handleTax }/>
            <label htmlFor="taxExclude" className="text-xs">
              税抜
            </label>
            <input type="radio" id="taxInclude" name="tax" value="税込" checked={taxNotation.tax === "税込"} onChange = { handleTax }/>
      ...
      <div className="mb-10">
        ...
        <div>
          <ul>
            {stocks.sort((a,b) => b.id - a.id).map((stock) => (
              <div key={stock.id}>
                {stock.type === "食品" && stock.use_date === null ? (
                  <Item props={stock} onDelete={del} onUpdate={update} date={date}/>
                ) : null}
              </div>
            ))}
          </ul>
